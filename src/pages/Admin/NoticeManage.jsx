...
const statusLabels = { published: '게시 중', draft: '임시 저장' }
    ...
    <section className={styles.page} aria-labelledby="notice-manage-title">
      ...
      <section className={styles.filterBar} aria-label="공지사항 검색 및 필터">
        <label className={styles.searchField}>
          ...
          <input type="search" value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} placeholder="제목 또는 내용 검색" />
        </label>
        <label className={styles.selectField}>
          <span className={styles.srOnly}>게시 상태</span>
          <select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)}>
            <option value="all">전체 상태</option><option value="published">게시 중</option><option value="draft">임시 저장</option>
          </select>
        </label>
        <label className={styles.selectField}>
          ...
          {isLoading ? (
            ...
            <div className={styles.tableWrap}>
              <table className={styles.noticeTable}>
                <thead><tr><th scope="col">공지 제목</th><th scope="col">상태</th><th scope="col">등록일</th><th scope="col">조회</th><th scope="col">관리</th></tr></thead>
                <tbody>
            ...
            <form onSubmit={handleSubmit}>
              <div className={styles.modalBody}>
                ...
                <div className={styles.formOptions}>
                  <label><span>게시 상태</span><select value={draft.status} onChange={(event) => setDraft((current) => ({ ...current, status: event.target.value }))}><option value="draft">임시 저장</option><option value="published">게시 중</option></select></label>
