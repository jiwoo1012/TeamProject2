...
const resolveAlcoholExplainImage = (imageUrl) =>
  Object.entries(alcoholExplainImages).find(([path]) =>
    path.endsWith(`/${imageUrl.split('/').pop()}`)
  )?.[1]

const seededStylingImages = [...stylingImages]
...
const ProductList = () => {
  ...
  const wishToastTimerRef = useRef(null)
  ...
  // 로그인한 사용자의 찜 목록을 Firestore에서 불러온다.
  useEffect(() => {
    ...
    getCollection(`users/${uid}/wishlist`)
      .then((docs) => {
        ...
        const productIds = docs
          .map((wishDoc) => wishDoc.productId ?? wishDoc.id)
          .filter(Boolean)

        setWishes(new Set(productIds))
      })
      .catch((error) => {
        console.error('찜 목록 조회 실패:', error)
        if (!isCancelled) setWishes(new Set())
      })
        ...
        : [
            ...new Set(
              activeCategory.data
                .map(
                  (item) =>
                    item[keys[categoryId]]
                )
                .filter(Boolean)
            ),
          ]

    if (categoryId === 'liquor') {
      values.push(
        '낮의 결',
        '밤의 결'
      )
  ...
  const alcoholExplain = [
    {
      ...
      explain:
        '시중에 파는 하얗고 걸쭉한 막걸리를 아시지요? 탁주는 쌀이나 밀로 만들어 맑게 거르지 않고, 우리 술의 진한 맛과 영양이 고스란히 담기도록 뽀얗게 빚어낸 친근한 술이랍니다.',

      flavor:
        '목넘김이 부드럽고 든든하며, 쌀 특유의 구수한 단맛과 감칠맛을 느낄 수 있습니다.',
...

    {
      ...
      explain:
        ' 예로부터 [몸에 이로운 좋은 술]이라는 뜻으로 부르던 맑은 술입니다. 앞서 말씀드린 탁주에서 맑은 윗부분만 조심스럽게 떠내어 깨끗하게 걸러낸 것이지요.',

      flavor:
        '색이 물처럼 아주 맑고 투명하며, 머리가 아프지 않고 깔끔하게 떨어지는 뒷맛이 일품이랍니다.',
...

    {
      ...
      flavor:
        '와인처럼 향이 매우 풍부하고, 입안에서 달콤새콤한 과일 맛이 퍼져 누구나 부담 없이 마실 수 있습니다.',

      recommend:
        '분위기 있는 저녁 식사나, 가벼운 디저트와 함께 달콤한 기분을 내고 싶으실 때 제격입니다.',
...

    {
      ...
      explain:
        '흔히 알고 계시는 [소주]의 전통 버전이라고 생각하시면 됩니다! 쌀이나 과일 등으로 만든 술을 불에 한 번 끓이고, 그 증기를 차갑게 식혀 맑은 눈물처럼 받아낸 도수 높은 술이옵니다.',

      flavor:
        '혀가 아플 정도로 쓴 인공적인 알코올 맛이 아니라, 원재료의 깊고 그윽한 풍미가 입안에 진하게 남는 것이 특징입니다.',

      recommend:
        '고기 요리처럼 기름진 음식을 드실 때, 입안을 개운하게 싹 씻어내고 싶으실 때 찾아주세요.',
...

    {
      ...
      explain:
        '전통주를 바탕으로 세상에 없던 다양한 허브, 향신료, 꽃 등을 더해 양조장 주인장의 개성을 듬뿍 담아 만든 독특한 술입니다.',

      flavor:
        '맛과 향이 매우 다채롭고 재미있어서, 마실 때마다 감탄이 절로 나오는 반전 매력이 있답니다.',

      recommend:
        '맨날 마시던 평범한 술 말고, 아주 색다른 경험을 해보고 싶으실 때 도전해 보세요!',
  ...
  return (
    <div className={styles.page}>
      ...
      >
        ...
        <section
          className={
            styles.stylingMarquee
          }
          aria-label="상품 스타일링 이미지"
        >
          <div
            className={
              styles.marqueeTrack
      ...
      {selectedAlcohol && (
        ...
        >
          <section
            ...
            role="dialog"
            aria-modal="true"
            aria-labelledby="alcohol-modal-title"
          >
            <button
              className={
                styles.modalClose
            ...
            >
              <img
                src={resolveAlcoholExplainImage(
                  selectedAlcohol.image
                )}
      ...
      {wishToast && (
        ...
        >
          ...
          <p>
            <strong>{wishToast.message}</strong>
          </p>

          <Link to={wishToast.linkTo}>
            {wishToast.linkText}{' '}
